    ...
    for item in snowflake_data_types:
        ...
        try:
            ...
            if converted is None:
                print(f"The datatype {field} is not a recognized snowflake datatype")
    ...
    try:
        ...
        for index, chunk in enumerate(
                pd.read_csv(
                    source_full_path,
                    chunksize=chunksize,
                    dtype=pandas_datatypes,
                    parse_dates=dates,
                )
        ):
            chunk.head(0).to_sql(
                table_name, con=db_connection, if_exists="fail", index=False
            )
...
def execute_copy_into_command(db_connection, table_name, results_dict):
    """
    ...
    copy_into_statement = (
        f"""COPY INTO "{table_name}" FILE_FORMAT='{format_name}' PURGE=TRUE"""
    )
...
def upload_data_with_put(
    ...
    try:
        ...
        if insert_method == "replace":
            ...
            snowflake_results = execute_copy_into_command(
                db_connection, table_name, snowflake_results
            )
        ...
        print(
            f'{source_full_path} successfully {insert_method}{"ed to " if insert_method == "append" else "d "}the table {table_name}.'
        )
    finally:
        os.remove(csv_path)
...
def upload_data_with_insert(
    ...
    for index, chunk in enumerate(
        ...
        chunk.to_sql(
            ...
            method="multi",
            # Not using pd_writer method due to inability to create tables and consistency with old behavior
            chunksize=10000,
        )
...
def upload_data(
    ...
    try:
        snowflake_results = upload_data_with_put(
            source_full_path,
            table_name,
            insert_method,
            db_connection,
        ...
        base_folder_name = shipyard.logs.determine_base_artifact_folder("snowflake")
        ...
        shipyard.files.write_json_to_file(
            snowflake_results, snowflake_upload_response_path
        )
        ...
        if "This session does not have a current schema." in str(pg_e):
            print(
                "The schema provided either does not exist or your user does not have access to it. If no schema was provided, no default schema exists."
            )
            print(pg_e)
            sys.exit(errors.EXIT_CODE_INVALID_SCHEMA)
        if "This session does not have a current database." in str(pg_e):
            print(
                "The database provided either does not exist or your user does not have access to it. If no database was provided, this user does not have a default warehouse."
            )
            print(pg_e)
    ...
    try:
        ...
        if "No active warehouse" in str(db_e):
            print(
                "The warehouse provided either does not exist or your user does not have access to it. If no warehouse was provided, this user does not have a default warehouse."
            )
        ...
        if "This session does not have a current schema." in str(db_e):
            ...
            print(db_e)
            sys.exit(errors.EXIT_CODE_INVALID_SCHEMA)
        print("Failed to upload file to Snowflake.")
        print(db_e.orig)  # Avoids printing data to console
        sys.exit(errors.EXIT_CODE_UNKNOWN_ERROR)
    except Exception as e:
        if "C error: Expected" and "fields" and "saw" in str(e):
            print(
                "File contents don't match the provided headers. One or more rows contain more columns than expected."
            )
            print(e)
...
def main():
    ...
    try:
        ...
        if args.user_role != "":
            db_params["role"] = args.user_role

        db_connection = create_engine(URL(**db_params))
    ...
    try:
        ...
        print(f'Failed to upload file to Snowflake. {e}')
    finally:
